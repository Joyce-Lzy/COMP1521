...
void access_page(int virtual_page, int access_time, int n_physical_pages, struct ipt_entry *ipt) {
    ...
    // The virtual page is not in a physical page
    for (int i = 0; i < n_physical_pages; i++) {
        if (ipt[i].virtual_page == -1) {
            // there is free physical page
    ...
    // there is no free physical page
    // the least-recently-used page is evicted
    int min_time = access_time;
    int j = 0;
    for (int i = 0; i < n_physical_pages; i++) {
        if (ipt[i].last_access_time <= min_time) {
            min_time = ipt[i].last_access_time;
            j = i;
        }  
    }
    printf("Time %d: virtual page %d  - virtual page %d evicted - loaded to physical page %d\n", access_time, virtual_page, ipt[j].virtual_page, j);
