...
static int check_redirection(char **words, char **path, char **environment, int length, posix_spawn_file_actions_t actions, int pipe_file_descriptors[2]);
...
static void execute_command(char **words, char **path, char **environment)
{
    ...
    if (has_redi != NOTHING) {
        ...
        check_redirection(words, path, environment, len_words, actions, pipe_file_descriptors);
...
static void pipe_line(char **s, char **path, char **environment) {
    ...
    for (int i = 0; i < total_s - 1; i++) {
        ...
        if (has_redi_out != NOTHING) {
            ...
            // READ redirection 
            check_error(outp, out_l);
        } 
        
        // read pipe, and put output in stdout   
...
//
// Find the full path can excute
//
static char *find_path(char *program, char **path) {
    char *file_path = NULL;
    if (strrchr(program, '/') == NULL) {
        // find the absolute path of program.
        char slash[2] = {'/'};
...
static pid_t input_file(char **words, char **path, char **environment, FILE *fr, posix_spawn_file_actions_t actions, int pipe_file_descriptors[2]) {
    ...
    if (is_executable(program)) {
        ...
        if (fw != NULL) {
            ...
            f = fdopen(pipe_file_descriptors[0], "r");
            if (f == NULL) {
                perror("fdopen");
                return NOTHING;
            }
